...
/**
  ...
  tokenRequester.getToken(baseUrl, settings)
    .then((token) => {
      console.log(token);
    }).catch((err) => {
      console.log('err', err);
...
/**
 Requests full Keycloak JSON response including Access Token, Refresh Token, expiries, etc
 @param {string} baseUrl - The baseurl for the Keycloak server - ex: http://localhost:8080/auth,
 @param {object} settings - an object containing the settings
 @param {string} settings.username - The username to login to the keycloak server - ex: admin
 @param {string} settings.password - The password to login to the keycloak server - ex: *****
 ...
 tokenRequester.getJSON(baseUrl, settings)
 .then((token) => {
      console.log(token);
    }).catch((err) => {
      console.log('err', err);
 ...
 */
function getJSON(baseUrl, settings) {
  return new Promise((resolve, reject) => {
    settings = settings || {};
